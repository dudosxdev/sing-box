...
type Server struct {
	...
	config    *kcp.Config
	reader    kcp.PacketReader
	maskMgr   *UdpmaskManager

	sync.Mutex
	sessions map[ConnectionID]*kcp.Connection
	udpConn  net.PacketConn
}

func NewServer(ctx context.Context, logger logger.ContextLogger, options option.V2RayMKCPOptions, tlsConfig tls.ServerConfig, handler adapter.V2RayServerTransportHandler) (adapter.V2RayServerTransport, error) {
	config := &kcp.Config{
		MTU:              options.MTU,
		TTI:              options.TTI,
	...
	return &Server{
		...
		maskMgr:   maskMgr,
		sessions:  make(map[ConnectionID]*kcp.Connection),
	}, nil
}
...
func (s *Server) ServePacket(listener net.PacketConn) error {
	var rawConn net.PacketConn = listener
	var err error

	if s.maskMgr != nil {
		rawConn, err = s.maskMgr.WrapPacketConnServer(rawConn)
		if err != nil {
			return err
		}
	}
...
func (s *Server) handlePackets(conn net.PacketConn) {
	...
	for {
		...
		if err != nil {
			...
			s.logger.Error("mkcp server read error: ", err)
			return
		}
	...
	if !found {
		...
		go func() {
			...
			if s.tlsConfig != nil {
				...
				if err != nil {
					s.logger.Error("mkcp tls handshake error: ", err)
					session.Close()
					return
				}
				netConn = tlsConn
