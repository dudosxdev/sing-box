...
func newError(msg string) error {
	return &kcpError{msg: msg}
}

type kcpError struct{ msg string }

func (e *kcpError) Error() string { return e.msg }
...
// semaphore is a simple channel-based semaphore
...
func (s *semaphore) Wait() <-chan struct{} { return s.token }
func (s *semaphore) Signal()              { s.token <- struct{}{} }
...
func (c *Connection) WriteMultiBuffer(mb MultiBuffer) error {
	reader := &MultiBufferContainer{MultiBuffer: mb}
	defer reader.Close()
...
func (c *Connection) writeMultiBufferInternal(reader io.Reader) error {
	...
	for {
		for {
			...
			if b == nil {
				b = buf.New()
				n, err := b.ReadFrom(io.LimitReader(reader, int64(c.mss)))
				if n == 0 || err != nil {
					b.Release()
					b = nil
					return nil
...
func (c *Connection) Input(segments []Segment) {
	...
	for _, seg := range segments {
		...
		case *CmdOnlySegment:
			...
			c.sendingWorker.ProcessReceivingNext(seg.SendingNext)
			c.receivingWorker.ProcessSendingNext(seg.ReceivingNext)
...
// NewRawConnection creates a new KCP connection (alias for NewConnection).
func NewRawConnection(meta ConnMetadata, writer io.Writer, closer io.Closer, config *Config) *Connection {
	conn := &Connection{
		meta:       meta,
		closer:     closer,
		since:      nowMillisec(),
